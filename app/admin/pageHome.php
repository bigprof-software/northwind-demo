	...
	<?php
		...
		while($row=db_fetch_row($res)) {
			...
			<tr>
				...
				<td class="remaining-width"><div class="clipped"><a href="pageEditOwnership.php?recID=<?php echo $row[3]; ?>"><img src="images/data_icon.gif" border="0" alt="<?php echo html_attr($Translation['view record details']); ?>" title="<?php echo html_attr($Translation['view record details']); ?>"></a> <?php echo getCSVData($row[0], $row[1]); ?></div></td>
			</tr>
			<?php
		}
	?>
			...
			<tr>
				...
				<td class="remaining-width">
					<a href="pageViewRecords.php?memberID=<?php echo urlencode($row[0] ? $row[0] : '{none}'); ?>">
						<img src="images/data_icon.gif" border="0" alt="<?php echo html_attr($Translation['view member records']); ?>" title="<?php echo html_attr($Translation['view member records']); ?>">
					</a>
...
<div class="panel panel-info">
	...
	<table class="table table-striped table-hover">
		<tr>
			...
			<td class="remaining-width"><a href="pageViewGroups.php"title="<?php echo html_attr($Translation['view groups']); ?>"><i class="glyphicon glyphicon-search"></i> <?php echo sqlValue("select count(1) from membership_groups"); ?></a></td>
			</tr>
		<tr>
			<th class=""><?php echo $Translation['active members']; ?></th>
			<td class="remaining-width"><a href="pageViewMembers.php?status=2" title="<?php echo html_attr($Translation['view active members']); ?>"><i class="glyphicon glyphicon-search"></i> <?php echo sqlValue("select count(1) from membership_users where isApproved=1 and isBanned=0"); ?></a></td>
			</tr>
		<tr>
			<?php
				$awaiting = intval(sqlValue("select count(1) from membership_users where isApproved=0"));
			?>
			<th class="" <?php echo ($awaiting ? "style=\"color: red;\"" : ""); ?>><?php echo html_attr($Translation['members awaiting approval']); ?></th>
			<td class="remaining-width"><a href="pageViewMembers.php?status=1" title="<?php echo html_attr($Translation['view members awaiting approval']); ?>"><i class="glyphicon glyphicon-search"></i> <?php echo $awaiting; ?></a></td>
			</tr>
		<tr>
			<th class=""><?php echo $Translation['banned members']; ?></th>
			<td class="remaining-width"><a href="pageViewMembers.php?status=3" title="<?php echo html_attr($Translation['view banned members']); ?>"><i class="glyphicon glyphicon-search"></i> <?php echo sqlValue("select count(1) from membership_users where isApproved=1 and isBanned=1"); ?></a></td>
			</tr>
		<tr>
			<th class=""><?php echo $Translation['total members']; ?></th>
			<td class="remaining-width"><a href="pageViewMembers.php" title="<?php echo html_attr($Translation['view all members']); ?>"><i class="glyphicon glyphicon-search"></i> <?php echo sqlValue("select count(1) from membership_users"); ?></a></td>
			</tr>
		</table>
	</div>
</div>
