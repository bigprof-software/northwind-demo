...
function employees_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$combo_Country->SelectedText = (isset($filterField[1]) && $filterField[1] == '14' && $filterOperator[1] == '<=>' ? $filterValue[1] : '');
	...
	$lookup_fields = ['ReportsTo' => ['employees', 'Reports To'], ];
	...
	$templateCode = str_replace('<%%UPLOADFILE(Photo)%%>', ($noUploads ? '' : "<div>{$Translation['upload image']}</div>" . '<i class="glyphicon glyphicon-remove text-danger clear-upload hidden"></i> <input type="file" name="Photo" id="Photo" data-filetypes="jpg|jpeg|gif|png|webp" data-maxsize="153600" accept=".jpg,.jpeg,.gif,.png,.webp">'), $templateCode);
